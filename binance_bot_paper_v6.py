...
class CarteiraMulti:
    ...
    def abrir(self, symbol, preco, conf):
        ...
        # Usa saldo ATUAL × TRADE_PCT — nunca trava por saldo insuficiente
        usdt = self.saldo * TRADE_PCT
        if usdt < 1:
            log.warning(f"⚠️  Saldo muito baixo para operar: {self.saldo:.2f} USDT")
            telegram(f"⚠️ <b>Saldo muito baixo!</b>\nSaldo atual: {self.saldo:.2f} USDT\nBot pausado.")
            return
        self.saldo -= usdt
        self.posicoes[symbol] = Posicao(symbol, preco, usdt)
        pos = self.posicoes[symbol]
        log.info(f"🟢 COMPRA {symbol} @ {preco:.4f} | Alocado: {usdt:.2f} USDT | SL:{pos.stop_loss:.4f} | SG:{pos.stop_gain:.4f}")
...
def main():
    ...
    telegram(
        f"🤖 <b>Crypto Bot v6.2 iniciado!</b>\n"
        f"📊 12 pares monitorados\n"

...
def avaliar_sinal(df: pd.DataFrame) -> tuple[str, dict]:
    ...
    # EMA é SEMPRE obrigatória — define a tendência
    # Dos outros 3 (RSI, Volume, MACD) exige pelo menos 2
    # Total mínimo: 3/4 com EMA confirmada
    ema_ok_buy  = ema_alta
    ema_ok_sell = ema_baixa
    ...
    # LONG: EMA obrigatória + pelo menos 2 dos outros 3
    if ema_ok_buy  and outros_buy  >= 2: return "LONG",  conf
    # SHORT: EMA obrigatória + pelo menos 2 dos outros 3
    if ema_ok_sell and outros_sell >= 2: return "SHORT", conf
    return "HOLD", conf
...
def main():
    ...
    log.info(f"  Sinal: EMA obrigatória + 2/3 (RSI, Volume, MACD)")
    ...
    try:
        while True:
            ...
            try:
                ...
                for sym in SYMBOLS:
                    try:
                        ...
                        log.info(f"   {sym:10} | {conf['ema']:5} | RSI:{conf['rsi']:5.1f} | "
                                 ...
                                 f"L:{conf['buy_score']}/4 S:{conf['sell_score']}/4 → {sinal}")
                ...
                if not sinais:
                    log.info("   ⏸️  Nenhum sinal (EMA + 2/3 restantes)")

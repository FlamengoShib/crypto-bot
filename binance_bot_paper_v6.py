"""
╔══════════════════════════════════════════════════════════╗
║      SENAC CRYPTO BOT — PAPER TRADING v6.1               ║
║   Usa CCXT — funciona em qualquer servidor sem bloqueio  ║
...
MUDANÇA v6.1:
  - Substituído python-binance por CCXT
  - CCXT não tem restrição geográfica — funciona no Railway EUA
  - Acessa dados públicos da Binance via CCXT sem bloqueio
  - Sem necessidade de API Key para paper trading
"""
...
def conectar():
    """
    CCXT acessa dados públicos da Binance sem restrição geográfica.
    Sem API Key necessária para paper trading.
    """
    ...
    log.info(f"✅ Conectado via CCXT — BTC: {ticker['last']:.2f} USDT")
    return exchange
